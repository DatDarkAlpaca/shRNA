...
def retrieve_gene_results(driver, gene_id):
    page_search = NCBIGeneSearch(driver, gene_id)
...
def retrieve_consensus(logger, gene_id):
    ...
    _, rna_temp_filename = save_temp_rna_file(rna_file)
...
def retrieve_target_and_tm(driver, consensus):
    si_crawler = SiDirectScrapper(driver)
...
def retrieve_variants_and_gene_name(gen_scrapper, sequence_list):
    ...
    for senso in sequence_list:
        result = gen_scrapper.get_sequence_variants(senso)
...
def without_external():
    ...
    rna_i_generator = 'SequenceParser'(target_sequences)
...
class SequenceInput(InputType):
    def get_input(self):
        return input('Enter the Gene\'s ID: ')
